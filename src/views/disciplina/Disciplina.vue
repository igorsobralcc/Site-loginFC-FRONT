<template>
  ...
  >
    ...
    >
      ...
      <v-toolbar-title>Tabela Disciplina</v-toolbar-title>
    </v-app-bar>
    <v-data-table
      :headers="headers"
      :items="disciplinas"
          ...
          >
            <v-card>
              ...
              <v-card-title class="headline">
                <strong>Tem certeza de que deseja excluir a disciplina</strong> <br><br> {{ disciplina }}?
              </v-card-title>
              <v-card-actions>
                <v-spacer />
                <v-btn
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteItem (item) {
        this.editedIndex = this.disciplinas.indexOf(item.id)
      ...
      async save () {
        const disciplinaId = this.editedItem.id
        const data = {
          nome: this.editedItem.nome,
        }
        const val = this.formTitle.indexOf('Edit') === 0 ? 'edit' : 'save'
        if (val === 'edit') {
          await DisciplinaDataService.update(disciplinaId, data)
            .then((res) => {
              this.alert = true
          ...
          await DisciplinaDataService.create(data)
            .then((res) => {
              this.alert = true
      ...
      async getDataFromApi () {
        ...
        const data = await DisciplinaDataService.getAllActive()
        console.log('listando: ', data)
        this.disciplinas = data.data
      },

      async deleteRow (item) {
        await DisciplinaDataService.delete(item.id)
          .then(() => {

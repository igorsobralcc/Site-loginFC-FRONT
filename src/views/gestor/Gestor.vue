0<template>
  ...
  >
    ...
    >
      ...
      <v-toolbar-title>Tabela Gestor</v-toolbar-title>
    </v-app-bar>
    <v-data-table
      :headers="headers"
      :items="gestores"
          ...
          >
            <v-card>
              ...
              >
                ...
                <v-toolbar-title>Deletar Gestor</v-toolbar-title>
              </v-app-bar>
          ...
          >
            <v-card>
              ...
              >
                ...
                <v-toolbar-title>Pesquisar Gestor</v-toolbar-title>
              </v-app-bar>
              <v-card-text>
                <v-container>
                  <v-row>
...
<script>
  import GestorDataService from '@/services/GestorDataService.js'
  export default {
    data: () => ({
      status: false,
      direction: 'top',
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Cadastrar Gestor' : 'Editar Gestor'
      },
    },
    ...
    methods: {
      ...
      deleteItem (item) {
        this.editedIndex = this.gestores.indexOf(item.id)
      ...
      async save () {
        const gestorId = this.editedItem.id
        const data = {
        ...
        if (val === 'edit') {
          await GestorDataService.update(gestorId, data)
            .then((res) => {
              this.alert = true
          ...
          await GestorDataService.create(data)
            .then((res) => {
              this.alert = true
      ...
      async deleteRow (item) {
        await GestorDataService.delete(item.id)
          .then(() => {
